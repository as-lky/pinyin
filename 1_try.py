...
def work(words):
    ...
    for i in range(3, num):
        ...
        # MM = min(MM)
        # dellist = []
        # for a, b in f[now ^ 1].items():
        #     if b > MM * (num / i):
        #         dellist.append(a)
        # for a in dellist:
        #     del f[now ^ 1][a]
        #     del ans[now ^ 1][a]
        for i1 in yin2pr[words[i - 3]]:
            for i2 in yin2pr[words[i - 2]]:
                for i3 in yin2pr[words[i - 1]]:
                    # if (i1, i2, i3) not in f[now ^ 1]:
                    #     continue
                    for i4 in yin2pr[words[i]]:
                        tmp = P(i1, i2, i3, i4)
                        if isclose(tmp, 0, rel_tol=eps):
                            tmp = eps
                        tmp = f[now ^ 1][(i1, i2, i3)] - log(tmp)
